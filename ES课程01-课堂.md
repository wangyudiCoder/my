...
* 关系型数据库
表扩展（减少冗余数据），字典表，多表联查，索引支持，事务支持；存储过程，触发器
问题：关系乱维护性差，关系多效率下降，大批量吞吐，事务处理影响效率，数据量受限等等

* 非关系型
文档型，key-value，列式，图形；
面向业务做针对性使用优化，减少了复杂不常用功能

* 缓存
MC，redis
常用热点数据，快速存取，异步操作，分布式锁等等
...
Elastic为主体的公司提供很多优秀方案，拿到了很多投资，现在已经上市了。
...
![image-20190308141458466](/Users/wangyudi/Library/Application Support/typora-user-images/image-20190308141458466.png)
...
* river:数据源的意思，比如可以直接从mysql获取数据，其他river官方还有couchDB，RabbitMQ等。2.0以后就弃用了，bug太多
...
中一个索引可能会分多个片，每个分片可以有自己的一个或多个副本（默认是5片1个副本）
...
![image-20190308003428670](/Users/wangyudi/Library/Application Support/typora-user-images/image-20190308003428670.png)
...
- 思考题
  对ES的工作模式优劣复习一下，最少三条优劣对比；思考下工作模式有什么优劣，有什么可以优化的地方？请最少列举出两条。

  根据我们讲解的bitmap，学习下布隆过滤器如何应用？

- 动手题
  搭建好ES单机环境（集群更好）和head插件。
